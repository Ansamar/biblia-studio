...
import { testVercelStegaRegex, stegaClean, v4, SharedStateContext, createOverlayController, sanityNodesExistInSameArray } from "./SharedStateContext.js";
import { decodeSanityNodeData, pathToUrlString } from "@sanity/visual-editing-csm";
...
import { getNodeIcon, InsertMenu, PopoverPortal, useTelemetry, PopoverBackground, PointerEvents, TelemetryContext, events } from "./InsertMenu.js";
...
function cleanStegaFromCopyEvent(event) {
  ...
  if (isTextField(activeElement)) {
    ...
    if (!text2 || !testVercelStegaRegex(text2)) return;
    event.preventDefault(), clipboardData.setData("text/plain", stegaClean(text2));
    return;
  }
  const selection = document.getSelection();
  if (!selection || selection.isCollapsed || selection.rangeCount === 0) return;
  const text = selection.toString(), html = serializeRangesToHtml(selection);
  !testVercelStegaRegex(text) && !testVercelStegaRegex(html) || (event.preventDefault(), clipboardData.setData("text/plain", stegaClean(text)), html && clipboardData.setData("text/html", stegaClean(html)));
}
...
function checkAttribute(element, attributeName, value, inHead, sink) {
  if (!(!value || !testVercelStegaRegex(value))) {
    if (inHead) {
...
function checkTextNode(node, inHead, sink) {
  ...
  !value || !testVercelStegaRegex(value) || sink(kind, parentElement ?? void 0, void 0, value);
}
...
function checkLocation(sink) {
  ...
  try {
    ...
    testVercelStegaRegex(decoded) && sink("url", void 0, void 0, decoded);
  } catch {
  }
}
...
function processMutation(mutation, sink) {
  ...
  if (!(!target.isConnected || isInsideOverlay(target)))
    if (type === "attributes") {
...
function decodeSanity(value) {
  try {
    ...
    return !decoded || typeof decoded != "object" || decoded.origin !== "sanity.io" ? void 0 : decodeSanityNodeData(decoded) ?? decoded;
  } catch {
    return;
  }
}
...
function observeSuspiciousStega(onSuspiciousStega) {
  ...
  const sink = (kind, element, attribute, value) => {
    const cleaned = stegaClean(value), key = `${kind}|${attribute ?? ""}|${cleaned}`;
...
const isReactElementOverlayComponent = (component) => isValidElement(component), Root$3 = /* @__PURE__ */ styled(Card).withConfig({
  displayName: "Root",
  componentId: "sc-e95gnt-0"
})(["background-color:var(--overlay-bg);border-radius:3px;pointer-events:none;position:absolute;will-change:transform;box-shadow:var(--overlay-box-shadow);transition:none;--overlay-bg:transparent;--overlay-box-shadow:inset 0 0 0 1px transparent;[data-overlays] &{--overlay-bg:color-mix(in srgb,transparent 95%,var(--card-focus-ring-color));--overlay-box-shadow:inset 0 0 0 2px color-mix(in srgb,transparent 50%,var(--card-focus-ring-color));}[data-fading-out] &{transition:box-shadow 1550ms,background-color 1550ms;--overlay-bg:rgba(0,0,255,0);--overlay-box-shadow:inset 0 0 0 1px transparent;}&[data-focused]{--overlay-box-shadow:inset 0 0 0 1px var(--card-focus-ring-color);}&[data-hovered]:not([data-focused]){transition:none;--overlay-box-shadow:inset 0 0 0 2px var(--card-focus-ring-color);}:link{text-decoration:none;}"]), Actions = /* @__PURE__ */ styled(Flex).withConfig({
  displayName: "Actions",
  componentId: "sc-e95gnt-1"
})(["bottom:100%;cursor:pointer;pointer-events:none;position:absolute;right:0;[data-hovered]:not([data-menu-open]) &{pointer-events:all;}[data-flipped] &{bottom:auto;top:100%;}"]), HUD = /* @__PURE__ */ styled(Flex).withConfig({
  displayName: "HUD",
  componentId: "sc-e95gnt-2"
})(["top:100%;cursor:pointer;pointer-events:none;position:absolute;left:0;gap:4px;padding:4px 0;flex-wrap:wrap;[data-hovered]:not([data-menu-open]) &{pointer-events:all;}[data-flipped] &{top:calc(100% + 2rem);}"]), MenuWrapper = /* @__PURE__ */ styled(Flex).withConfig({
  displayName: "MenuWrapper",
  componentId: "sc-e95gnt-3"
})(["margin:-0.5rem;[data-hovered]:not([data-menu-open]) &{pointer-events:all;}"]), Tab = /* @__PURE__ */ styled(Flex).withConfig({
  displayName: "Tab",
  componentId: "sc-e95gnt-4"
})(["bottom:100%;cursor:pointer;pointer-events:none;position:absolute;left:0;[data-hovered]:not([data-menu-open]) &{pointer-events:all;}[data-flipped] &{bottom:auto;top:100%;}"]), ActionOpen = /* @__PURE__ */ styled(Card).withConfig({
  displayName: "ActionOpen",
  componentId: "sc-e95gnt-5"
})(["cursor:pointer;background-color:var(--card-focus-ring-color);right:0;border-radius:3px;& [data-ui='Text']{color:#fff;white-space:nowrap;}"]), Labels = /* @__PURE__ */ styled(Flex).withConfig({
  displayName: "Labels",
  componentId: "sc-e95gnt-6"
})(["display:flex;align-items:center;background-color:var(--card-focus-ring-color);right:0;border-radius:3px;& [data-ui='Text'],& [data-sanity-icon]{color:#fff;white-space:nowrap;}"]), ExclusivePluginContainer = /* @__PURE__ */ styled.div.withConfig({
...
})(["position:fixed;bottom:2rem;left:2rem;"]), OverlayMinimapPrompt = () => {
  const $ = c(2);
  let t0;
...
function usePerspectiveSync(comlink2, dispatch, onPerspectiveChange, onVariantChange) {
  const $ = c(14), handlesPerspectiveChange = !!onPerspectiveChange, handlesVariantChange = !!onVariantChange;
  let t0;
  ...
  let t2;
  $[9] !== comlink2 || $[10] !== dispatch || $[11] !== handlesPerspectiveChange || $[12] !== handlesVariantChange ? (t2 = [comlink2, dispatch, handlesPerspectiveChange, handlesVariantChange], $[9] = comlink2, $[10] = dispatch, $[11] = handlesPerspectiveChange, $[12] = handlesVariantChange, $[13] = t2) : t2 = $[13], useEffect(t1, t2);
}
function _temp$5() {
}
...
function useReportDocuments(comlink2, elements, perspective, variant) {
  const $ = c(8), lastReported = useRef(void 0);
  let t0;
  ...
  }, t2 = [elements, perspective, variant, reportDocuments], $[2] = elements, $[3] = perspective, $[4] = reportDocuments, $[5] = variant, $[6] = t1, $[7] = t2) : (t1 = $[6], t2 = $[7]), useEffect(t1, t2);
}
...
function Overlays(props) {
  ...
  }, t3 = [comlink2], $[1] = comlink2, $[2] = t2, $[3] = t3) : (t2 = $[2], t3 = $[3]), useEffect(t2, t3), usePerspectiveSync(comlink2, dispatch, onPerspectiveChange, onVariantChange), useReportDocuments(comlink2, elements, perspective, variant);
  let t4;
...
const LoaderComlink = lazy(() => import("./LoaderComlink.js")), VisualEditing = (props) => {
  ...
  }, t4 = [keepStegaOnCopy], $[2] = keepStegaOnCopy, $[3] = t3, $[4] = t4) : (t3 = $[3], t4 = $[4]), useEffect(t3, t4);
  let t5;
  $[5] !== onSuspiciousStega ? (t5 = (reports) => onSuspiciousStega?.(reports), $[5] = onSuspiciousStega, $[6] = t5) : t5 = $[6];
  const handleSuspiciousStega = useEffectEvent(t5), hasSuspiciousStegaCallback = typeof onSuspiciousStega == "function";
  let t6;
  $[7] !== handleSuspiciousStega || $[8] !== hasSuspiciousStegaCallback ? (t6 = () => {
    if (hasSuspiciousStegaCallback)
      return observeSuspiciousStega(handleSuspiciousStega);
  }, $[7] = handleSuspiciousStega, $[8] = hasSuspiciousStegaCallback, $[9] = t6) : t6 = $[9];
  let t7;
  $[10] !== hasSuspiciousStegaCallback ? (t7 = [hasSuspiciousStegaCallback], $[10] = hasSuspiciousStegaCallback, $[11] = t7) : t7 = $[11], useEffect(t6, t7);
  ...
  }, t9 = [portal], $[12] = portal, $[13] = t8, $[14] = t9) : (t8 = $[13], t9 = $[14]), useEffect(t8, t9);
  ...
  $[15] !== comlinkStatus || $[16] !== inFrame || $[17] !== inPopUp ? (t10 = () => {
    ...
    return _temp3;
  }, t11 = [comlinkStatus, inPopUp, inFrame], $[15] = comlinkStatus, $[16] = inFrame, $[17] = inPopUp, $[18] = t10, $[19] = t11) : (t10 = $[18], t11 = $[19]), useEffect(t10, t11);
  let t12;
  $[20] !== comlink2 || $[21] !== comlinkStatus || $[22] !== components || $[23] !== inFrame || $[24] !== inPopUp || $[25] !== onPerspectiveChange || $[26] !== onVariantChange || $[27] !== plugins || $[28] !== zIndex ? (t12 = inFrame !== null && inPopUp !== null && /* @__PURE__ */ jsx(Overlays, { comlink: comlink2, comlinkStatus, componentResolver: components, onPerspectiveChange, onVariantChange, plugins, inFrame, inPopUp, zIndex }), $[20] = comlink2, $[21] = comlinkStatus, $[22] = components, $[23] = inFrame, $[24] = inPopUp, $[25] = onPerspectiveChange, $[26] = onVariantChange, $[27] = plugins, $[28] = zIndex, $[29] = t12) : t12 = $[29];
  let t13;
  $[30] !== comlink2 || $[31] !== history || $[32] !== refresh ? (t13 = comlink2 && /* @__PURE__ */ jsxs(Fragment, { children: [
  ...
  let t14;
  $[34] !== comlinkStatus || $[35] !== hasQueryListeners2 ? (t14 = comlinkStatus === "connected" && hasQueryListeners2 && /* @__PURE__ */ jsx(Suspense, { children: /* @__PURE__ */ jsx(LoaderComlink, {}) }), $[34] = comlinkStatus, $[35] = hasQueryListeners2, $[36] = t14) : t14 = $[36];
  let t15;
  $[37] !== t12 || $[38] !== t13 || $[39] !== t14 ? (t15 = /* @__PURE__ */ jsxs(Fragment, { children: [
    t12,
    t13,
    t14
  ...
  if (portal === !1 || !portalElement)
    return children;
  let t16;
  return $[41] !== children || $[42] !== portalElement ? (t16 = createPortal(children, portalElement), $[41] = children, $[42] = portalElement, $[43] = t16) : t16 = $[43], t16;
};
